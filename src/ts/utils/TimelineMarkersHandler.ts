...
export class TimelineMarkersHandler {
  ...
  public setActiveMarker(activeMarker: SeekBarMarker | null): void {
    ...
    this.timelineMarkers.forEach(marker => {
      if (marker.element) {
        marker.element.removeClass(prefixCss('seekbar-marker-active'));
    ...
    if (activeMarker && activeMarker.element) {
      activeMarker.element.addClass(prefixCss('seekbar-marker-active'));
    }
  }
  ...
  public updateActiveMarkerForPlayback(currentTime: number): void {
    ...
    const activeMarker = this.timelineMarkers.find(marker => {
      const markerTime = getMarkerTime(marker.marker, this.player, this.player.getDuration(), this.getSeekableRangeRespectingSnapshot());
      const hasDuration = marker.duration > 0;
